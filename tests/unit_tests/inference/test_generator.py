...
@pytest.fixture
def tokenizer():
    dataset_path = get_dataset_path("tinyshakespeare")
    with open(dataset_path["train"], "r", encoding="utf-8") as f:
        text = f.read()
...
def make_model(tokenizer):
    def _make_model(vocab_size=None, block_size=16, device="cpu"):
...
def get_generator(model, tokenizer, ckpt_path=None, device="cpu"):
    ...
    return Generator(config, model, tokenizer, device=torch.device(device))
