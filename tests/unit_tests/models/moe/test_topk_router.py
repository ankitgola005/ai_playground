...
def test_topk_correctness(B, T, D, E, K):
    ...
    logits = router.linear(x)
    probs = torch.softmax(logits, dim=-1)

    idx, vals = router(x)

    true_vals, true_idx = torch.topk(probs, k=K, dim=-1)

    assert torch.allclose(vals, true_vals, atol=1e-6)
    assert torch.equal(idx, true_idx)
